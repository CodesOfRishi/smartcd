...
if [[ ${smartcd_finder} = *fdfind || ${smartcd_finder} = *fd ]]; then
	...
	local find_base_dir_cmd_args="${smartcd_finder} \${SMARTCD_BASE_DIR} -type d ! -path '*/\.git/*' 2> /dev/null | ${smartcd_grep} -v '\.git$'"
	local find_sub_dir_cmd_args="${smartcd_finder} . -type d ! -path '*/\.git/*' 2> /dev/null | ${smartcd_grep} -v '\.git$'"
